...
const createReviewsHeader = () => {
  let reviewStream = fs.createWriteStream(__dirname + '/reviews.csv');
  reviewStream.write('title,contents,stars,user,experience,dateSubmitted,location,upVotes,downVotes,pros,cons,wouldRecommend\n');
}

const addReviewsToCSV = () => {
  let reviewsStream = fs.createWriteStream(__dirname + '/reviews.csv', {flags: 'a'});
  for (let i = 0; i < reviews.length; i++) {
    reviewsStream.write(`${reviews[i].title},${reviews[i].contents},${reviews[i].stars},${reviews[i].user},${reviews[i].experience},${reviews[i].dateSubmitted},${reviews[i].location},${reviews[i].upVotes},${reviews[i].downVotes},${reviews[i].pros},${reviews[i].cons},${reviews[i].wouldRecommend}\n`);
  }
}

const createReviewsCSV = () => {
  createReviewsHeader();
  addReviewsToCSV();
};
